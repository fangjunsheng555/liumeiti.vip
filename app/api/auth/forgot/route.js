...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f4f6fb;padding:32px 16px;">
  <tr><td align="center">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:480px;background:#fff;border-radius:18px;overflow:hidden;box-shadow:0 8px 32px rgba(15,23,42,0.06);">
      ${buildEmailBrandHeader({ brandName: BRAND_NAME, siteDomain: SITE_DOMAIN, label: L("密码重置", "Password Reset") })}
      <tr><td style="padding:30px 32px 14px;">
        ...
        <p style="margin:0 0 18px;font-size:13.5px;color:#475569;line-height:1.7;">${L(`您正在重置 ${BRAND_NAME} 账号密码。请在 10 分钟内输入下方验证码完成重置。如果不是您本人操作,请忽略此邮件`, `You're resetting your ${BRAND_NAME} account password. Enter the code below within 10 minutes to finish. If this wasn't you, please ignore this email.`)}</p>
    ...
    const text = L(`${BRAND_NAME} 密码重置\n\n验证码: ${code}\n有效期 10 分钟\n\n如非本人操作,请忽略此邮件`, `${BRAND_NAME} password reset\n\nCode: ${code}\nValid for 10 minutes\n\nIf this wasn't you, please ignore this email.`);
    ...
    const result = await sendSimpleEmail({
      to: email,
      subject: L(`${BRAND_NAME} · 密码重置验证码 ${code}`, `${BRAND_NAME} · Password reset code ${code}`),
      text, html,
    });
    if (!result.ok) {
