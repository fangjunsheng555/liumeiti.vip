...
export async function POST(request, { params }) {
  ...
  if (!adminPermissionProfile(session).canSendRedeemCodes) {
  ...
  const brandName = (locale === "en" ? settings.brand.nameEn : settings.brand.name)
    || (locale === "en" ? "Maoyang Taiwan Inc" : "冒央会社");
  ...
  const serviceText = (info.services || [])
    .map((s) => s.label || s.name || s.id)
    .filter(Boolean)
    .join(" + ");
  const previewLine = info.type === "service"
    ? `服务码 ${info.code}：${serviceText || "服务兑换码"}`
