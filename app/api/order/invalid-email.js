...
function orderItems(order) {
  return Array.isArray(order?.items) && order.items.length > 0
    ? order.items
    : [{ label: order?.serviceLabel || "订单服务", cycle: order?.cycle || "1年", amount: order?.finalAmount || 0 }];
}

export function buildInvalidOrderEmailHtml({ order, brandName, siteDomain, siteUrl, supportContact }) {
  ...
  const itemsRows = items.map((item, index) => `
    <tr>
      <td style="padding:12px 0;border-bottom:${index === items.length - 1 ? "0" : "1px solid #edf2f7"};">
        <div style="font-size:14px;font-weight:800;color:#0f172a;line-height:1.45;">${escapeHtml(item.label || "订单服务")}</div>
        <div style="margin-top:3px;font-size:12.5px;color:#64748b;">${escapeHtml(item.cycle || "1年")} · ${formatMoney(item.amount || 0)}</div>
      </td>
    </tr>`).join("");
...
<html lang="zh-CN">
  <head>
    ...
    <title>订单未收到付款 - ${escapeHtml(brandName)}</title>
  </head>
  <body style="margin:0;background:#f3f7fb;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI','PingFang SC','Microsoft YaHei',Arial,sans-serif;color:#0f172a;">
    <div style="display:none;max-height:0;overflow:hidden;">${escapeHtml(brandName)} 订单 ${escapeHtml(order.orderId)} 暂未收到付款，订单已标记为无效</div>
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:580px;background:#ffffff;border-radius:20px;overflow:hidden;border:1px solid #dbe7ef;box-shadow:0 16px 40px rgba(15,23,42,.08);">
            ...
            <tr>
              <td style="padding:28px 24px 8px;text-align:center;">
                ...
                <h1 style="margin:16px 0 8px;font-size:24px;line-height:1.25;font-weight:900;color:#0f172a;letter-spacing:-.03em;">未收到付款，订单已无效</h1>
                <p style="margin:0;color:#475569;font-size:14px;line-height:1.75;">我们暂未收到此订单对应付款，系统已将订单状态更新为无效。若您已经完成付款，请及时联系在线客服核对。</p>
              </td>
            </tr>
            <tr>
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-radius:16px;background:#fff7ed;border:1px solid #fed7aa;">
                  <tr>
                    <td style="padding:15px 16px;">
                      <div style="font-size:11px;color:#c2410c;font-weight:800;letter-spacing:.12em;text-transform:uppercase;">订单号</div>
                    ...
                    <td style="padding:15px 16px;text-align:right;border-left:1px solid #fed7aa;">
                      <div style="font-size:11px;color:#c2410c;font-weight:800;letter-spacing:.12em;text-transform:uppercase;">订单状态</div>
                      <div style="margin-top:3px;font-size:15px;font-weight:900;color:#c2410c;">无效 · 未收到付款</div>
                    </td>
                  </tr>
                </table>
              </td>
            ...
            <tr>
              <td style="padding:22px 24px 0;">
                <div style="font-size:11px;color:#94a3b8;font-weight:800;letter-spacing:.14em;text-transform:uppercase;margin-bottom:8px;">订单内容</div>
            ...
            <tr>
              <td style="padding:22px 24px 0;">
                <div style="border-radius:16px;background:linear-gradient(135deg,#f0fdfa,#ecfeff);border:1px solid #a7f3d0;padding:16px;">
                  <div style="font-size:14px;font-weight:900;color:#0f766e;margin-bottom:8px;">如果您已经付款</div>
                  <p style="margin:0;font-size:13.5px;line-height:1.8;color:#134e4a;">请携带付款凭证、订单号与下单邮箱联系在线客服，我们会尽快为您核对并继续处理。</p>
            ...
            <tr>
              <td style="padding:22px 24px 0;">
                <a href="${escapeHtml(queryUrl)}" style="display:block;text-align:center;text-decoration:none;border-radius:14px;background:#0f172a;color:#ffffff;font-size:14px;font-weight:900;padding:14px 16px;">查看订单状态</a>
              </td>
            </tr>
            <tr>
...
export function buildInvalidOrderEmailText({ order, brandName, siteDomain, siteUrl, supportContact }) {
  const queryUrl = `${siteUrl || "https://" + (siteDomain || "")}/service-center?order=${encodeURIComponent(order.orderId)}`;
  const lines = [
    `${brandName} - 未收到付款，订单已无效`,
    "",
    `订单号: ${order.orderId}`,
    "状态: 无效 · 未收到付款",
    `查询: ${queryUrl}`,
  ...
  orderItems(order).forEach((item) => {
    lines.push(`- ${item.label || "订单服务"} (${item.cycle || "1年"}) ${formatMoney(item.amount || 0)}`);
  });
  lines.push(
    "",
    "我们暂未收到此订单对应付款，系统已将订单状态更新为无效。",
    "如果您已经付款，请携带付款凭证、订单号与下单邮箱联系在线客服核对。",
    "",
    supportContact || "",
