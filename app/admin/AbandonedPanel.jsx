...
export default function AbandonedPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      else if (r.status === 401) setMsg("无权限（仅超级管理员）");
    } catch (e) { setMsg("加载失败"); }
  ...
  async function act(id, action) {
    ...
    try {
      ...
      const d = await r.json();
      if (d && d.ok) { setMsg(action === "email" ? "✅ 召回邮件已发送" : "✅ 已标记成交"); load(); }
      else setMsg(d.error === "no_email" ? "该弃单没有邮箱，无法发信召回" : (d.error === "send_failed" ? "发信失败，请重试" : "操作失败"));
    } catch (e) { setMsg("操作失败"); }
    setBusy("");
  }
  async function delSelected() {
    ...
    setBusy("del"); setMsg("");
    try { const r = await fetch("/api/admin/abandoned", { method: "DELETE", credentials: "same-origin", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ids: [...selected] }) }); const d = await r.json(); setMsg(d && d.ok ? `已删除 ${d.deleted} 条` : "删除失败"); } catch (e) { setMsg("删除失败"); }
    setBusy(""); load();
  }
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td style={{ ...td, textAlign: "center", color: C.muted }} colSpan={8}>暂无弃单（很好，到结算的都下单了）</td></tr>
            ) : rows.map((r) => {
