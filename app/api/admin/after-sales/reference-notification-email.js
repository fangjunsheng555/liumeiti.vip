...
function latestItems(order) {
  ...
  return source.map((item) => ({
    ...
    account: item?.staffAccount || item?.account || (source.length === 1 ? order?.staffAccount || order?.account || "" : ""),
    password: item?.staffPassword || item?.password || (source.length === 1 ? order?.staffPassword || order?.password || "" : ""),
...
function credentialSection(order, en) {
  ...
  const rows = latestItems(order).map((item) => {
    const accountLabel = item.service === "rocket" ? L("用户名", "Username") : L("账号", "Account");
    const details = [
...
export function buildReferenceNotificationEmail({ orders, subject, message, brandName, siteDomain, locale = "zh" }) {
  ...
  const orderRows = orders.map((order) => {
    ...
    const services = latestItems(order).map((item) => item.label).filter(Boolean).join(" / ");
    return `<tr><td bgcolor="#ffffff" style="padding:22px 0;border-top:1px solid #dbe4ea;background:#ffffff!important;">
      ...
      ${noteSection(L("订单备注", "Service note"), order.staffNotes)}
  ...
  <body bgcolor="#ffffff" style="margin:0;background:#ffffff!important;font-family:-apple-system,BlinkMacSystemFont,'PingFang SC','Microsoft YaHei',Arial,sans-serif;color:#0f172a!important;">
    <div style="display:none;max-height:0;overflow:hidden;color:#ffffff;">${esc(safeSubject)} · ${esc(message)}</div>
  ...
  for (const order of orders) {
    ...
    for (const item of latestItems(order)) {
      ...
      if (item.account) text.push(`${item.service === "rocket" ? L("用户名", "Username") : L("账号", "Account")}: ${item.account}`);
    ...
    if (order.staffNotes) text.push(`${L("订单备注", "Service note")}: ${order.staffNotes}`);
